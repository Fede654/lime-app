...
auto_detect_image_config() {
    ...
    local libremesh_rootfs="$LIME_PACKAGES_DIR/build/libremesh-2024.1-ow23.05.5-default-x86-64-generic-squashfs-rootfs.img.gz"
...
validate_image_config() {
    ...
    if [[ "$IMAGE_TYPE" == "librerouteros-2410" ]]; then
        if ! tar -tf "$ROOTFS_PATH" >/dev/null 2>&1; then
            print_error "LibreRouterOS rootfs must be extractable tar.gz format"
            exit 1
        fi
    fi
...
show_available_configs() {
    ...
    echo ""
    echo "Usage:"
    echo "  npm run qemu:start                          # Auto-detect (prefers stable)"
    echo "  QEMU_IMAGE_CONFIG=libremesh-2305 npm run qemu:start"
    echo "  QEMU_IMAGE_CONFIG=librerouteros-2410 npm run qemu:start"
}
