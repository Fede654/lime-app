...
- **Configuration Selection** - Environment variables and npm scripts working
...
./scripts/qemu-dev-fix.sh cleanup
```

### **LibreRouterOS Investigation**
```bash
# Test LibreRouterOS boot in foreground
cd ../lime-packages
timeout 30 sudo ./tools/qemu_dev_start --verbose --libremesh-workdir . \\
  build/librerouteros-24.10.1-r28597-0425664679-x86-64-rootfs.tar.gz \\
  build/librerouteros-24.10.1-r28597-0425664679-x86-64-generic-kernel.bin

# Check build contents
tar -tf build/librerouteros-24.10.1-r28597-0425664679-x86-64-rootfs.tar.gz | head -20
```
...
**Immediate recommendation:** Use `npm run qemu:start:libremesh` for current lime-app development while we investigate the LibreRouterOS kernel boot process.