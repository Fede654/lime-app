...
export const Setting = ({ expectedHost, expectedNetwork, delay = 1000 }) => {
    ...
    function runProgress(totalTime, cb) {
        ...
        if (progress < 100)
            setProgress((progress) => progress + 100 / totalTime);
        if (time <= 1) cb();
    }
        ...
        <div className="container container-padded">
            {state.action !== "finish" ? (
                    ...
                    {state.action === "setting" ? (
                        <div>
                            ...
                            <div style={{ width: "100%" }}>
                                ...
                                >
                                    <Trans>Please wait {time} seconds</Trans>
                                </span>
                            </div>
                        </div>
                    ) : (
                    ...
                    {state.action === "checking" ? (
                        <div>
                            ...
                            {state.notOnNetwork ? (
                                <p>
                                    <Trans>
                                        You are connected to another node in the
                                        network, try connecting to
                                    </Trans>{" "}
                                    {expectedNetwork}/{expectedHost}
                                </p>
                            ) : (
                                <p>
                                    <Trans>
                                        You should try to connect to the network{" "}
                                        {wifiSsid}.
                                    </Trans>
                                </p>
                            )}
                ...
                <div>
                    ...
                    <div>
                        <p>
                            <Trans>You have successfuly connected to</Trans>{" "}
                            {state.hostname}
                        </p>
                        <p>
                            <Trans>You are now part of </Trans>{" "}
                            {expectedNetwork}
                        </p>
